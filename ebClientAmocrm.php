...
class EbClientAmocrm{
    ...
    private function request($link, $p = false,  $body=false, $headers = false){
    ...
    public function update_lead($body){
        $request = $this::request('https://' . $this->config['client_domen'] . '.amocrm.ru/api/v2/leads', true, $body, false);
        return $request;
    }
    ...
    public function get_notes($type = false, $id = false){
        $headers = ['Authorization: Bearer ' . $this->access, "IF-MODIFIED-SINCE: " . date("D, d M Y H:i:s", time() - 10 * 60)]; 
        $link = 'https://' . $this->config['client_domen']  . '.amocrm.ru/api/v2/notes?type='. $type;
        if($id) $link.='&element_id='. $id;
        return $this::request($link, false, false, $headers)[1]['_embedded']['items'];
    }
